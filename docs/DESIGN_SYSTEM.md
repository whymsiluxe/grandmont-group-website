...
Конкретные HEX/шрифты ниже — [REC], предложение команды разработки на основе анализа референсов. Финальное утверждение — на Phase 2 (Design proof), когда владелец увидит реальный макет и подтвердит go/no-go.
...
**[DECIDE]** Точный HEX акцентного цвета для Grandmont Group — определить на Phase 1/2 design-proof, не переносить палитры других брендов без отдельного решения.
...
## 4.1 Реальная фотография — ТОЛЬКО в /projekte [РЕШЕНО юзером 2026-09-24]

**Homepage и service-hero НЕ требуют реальной фотографии.** Это не временный
placeholder в ожидании клиентских фото — это осознанная арт-дирекция.
Visual language главной страницы и верха service-страниц: abstract motion +
типографика + depth-эффекты на основе approved Refero-референсов (Sequel/
ThoughtLab/Vivid+Co/Lusion/Hyper Tria/Art+Commerce/Cosmos), НЕ витрина
фотографий стройки.

Реальная клиентская/проектная фотография зарезервирована исключительно
для `/projekte` → карточки проектов → `/projekte/[slug]` → галерея/
before-after/детали объекта. Отсутствие фото на Homepage/service-hero —
НЕ технический долг и НЕ пункт "недостающий контент" в аудитах. При
финальном визуальном аудите проверять не "есть ли фото", а "достаточно
ли настоящих эффектных motion-сцен, чтобы сайт не выглядел как крупный
текст на чёрном фоне".

## 5. Верх страницы услуги — ОБЯЗАТЕЛЬНЫЙ ПАТТЕРН [REQ, план блок 6.1]

**Block 1 (Hero, dark):**
- Abstract/motion visual фон (НЕ фото — см. §4.1), может использовать типографику/depth/форму как основной визуальный язык
