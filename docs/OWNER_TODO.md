...
- [x] **Баг в CRM: photo-upload сломан** (найден 2026-09-25, **пофикшен 2026-09-25** в `grandmont-group-crm`) — `backend/storage_service.py` требовал `EMERGENT_LLM_KEY` (унаследовано от Emergent-платформы), переписан на локальное filesystem-хранилище (`backend/storage/`, chmod 600, path-traversal защита). Проверено: `POST /api/objects/{id}/photos` → 200, `GET /api/files/{path}` → 200 байт-в-байт. Website-адаптер не менялся (не требовалось). Подробности — `docs/CRM_INTEGRATION_NOTES.md`. **Дальше**: повторно прогнать тестовый лид с фото end-to-end.
